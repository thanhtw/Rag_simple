...
class DataProcessor:
    ...
    def _get_chunker_index(self, chunk_options):
        ...
        """
        if (st.session_state.get("llm_choice") == self.llm_options["Online"] and 
            not st.session_state.get("llm_api_key") and 
            st.session_state.get("chunkOption") == "AgenticChunker"):
    ...
    def _get_chunks(self, text, chunk_option, embedding_option=None):
        ...
        if chunk_option == NO_CHUNKING:
            ...
            llm_model = (st.session_state.get("llm_model") 
                        ...
                        else st.session_state.get("local_llms"))
            if llm_model:
    ...
    def _render_collection_manager(self):
        ...
        # Preview section based on selected collection
        #self._render_collection_preview()
    ...
    def _render_collection_preview(self):
        """Render the preview of the selected collection."""
        if st.session_state.preview_collection and st.session_state.preview_collection != "None":
            st.divider()
            with st.container():
                st.write(f"**Preview of {st.session_state.preview_collection}**")
                self._preview_collection(st.session_state.preview_collection)
            st.divider()
